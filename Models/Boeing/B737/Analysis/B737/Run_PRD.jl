...
function PayloadRange()
    ...
    # Load default model
    ac = TASOPT.read_aircraft_model(joinpath(TASOPT.__TASOPTroot__, "..//Models/Boeing/B737/Analysis/B737/B738_input_prd.toml"))
    ...
    # Set the DPI, font size, font family, and figure size
    rc("figure", dpi=300)  # Set the resolution
    rc("font", size=14)  # Set the global font size
    rc("font", family="Times New Roman")  # Set the font family
    figure(figsize=(8, 6))  # Width, height in inches

    y_OEW = y_Wemtpy .+ y_payload
    plot(x_range ./ (1000*1852.0), y_OEW./ (9.8*1000), linestyle="-", label="OEW + Payload ", linewidth=2)

    xlabel("Range (1000 nmi)", fontsize=14)
    ylabel("Weight (1000 kg)", fontsize=14)
    title("Payload Range Diagram: 737-900", fontsize=16, fontweight="bold")
    legend()
    ...
    ax[:set_xlim]([min_value_x, max_value_x])  # Set x-axis limits
    ax[:set_ylim]([min_value_y, max_value_y])  # Set y-axis limits

    # Customizing the grid line style, width, and color
    grid(true, linestyle=":", linewidth=0.1, color="gray")

    # Set font size for tick labels
    ax = gca()  # Get the current axes
    ax[:tick_params](axis="both", which="major", labelsize=12)  # Adjust major tick label size
    ax[:tick_params](axis="both", which="minor", labelsize=12)  # Adjust minor tick label size

    # Enable and set minor ticks
    ax[:minorticks_on]()  # Turn on minor ticks

    # Customizing the minor grid
    # Note: Minor grid customization must be done after enabling minor ticks
    ax[:grid](true, which="minor", linestyle=":", linewidth=0.1, color="gray")

    # Importing AutoMinorLocator from Matplotlib's ticker module
    ticker = PyPlot.matplotlib[:ticker]
    ax[:xaxis][:set_minor_locator](ticker.AutoMinorLocator())  # Set minor ticks for the x-axis
    ax[:yaxis][:set_minor_locator](ticker.AutoMinorLocator())  # Set minor ticks for the y-axis


    savefig("Plots/B739_PRD_full_fuel.png")
